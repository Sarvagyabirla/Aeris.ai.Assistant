...
def test_context_manager(tmp_path):
    ...
    # Override persistent memory with a test instance
    filepath = tmp_path / "memory.json"
    cm.persistent_memory = PersistentMemory(str(filepath))

    cm.persistent_memory.add_fact("The user likes Python.")

    sys_inst = cm.get_system_instruction()
    assert "You are Aeris" in sys_inst
    assert "The user likes Python." in sys_inst
